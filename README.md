...
- **AI duplicate issue detection (RAG)** — new issues are embedded (Google Gemini embeddings) and matched via semantic search (Pinecone, per-repo namespaces); the top candidates then ground a *generative* Gemini verdict that flags duplicates and explains why. Debounced 600ms on the frontend + 5-min result caching to keep costs in check.
...
| **AI** | Google Gemini embeddings (768-dim) · Gemini generative verdicts · Pinecone vector store |
